...
!
!		Modified by AJN 2013/04 to stack the ray frame polarisation
!===============================================================================
   program sheba_stack
!===============================================================================
   use array_sizes
...
!    load error surfaces from files listed in sheba_stack.in
!
!	Modified AJN 2010/01
!	Now reads azimuth, backazimuth from .lam2 file and stacks dt v phi''
!	(phi at the source, rather than receiver; phi''=az+baz-phi)
!	
    use array_sizes
!-------------------------------------------------------------------------------
   implicit none
      ...
      integer :: iwmode ! 1=even weighting, 2=snr weighting, 3=file of weights
      real :: az,baz  ! AJN: Read in from .lam2 file 
      integer :: fast_shift ! AJN: (az+baz) % 180
...
! MODIFIED AJN 2013/04         
         fast_shift=int(modulo(baz,180.))
         do i=fast_shift,1,-1
         	read(20,*) (error_in(i,j,nsurf),j=1,n2)
         enddo
         do i=n1,fast_shift+1,-1
            read(20,*) (error_in(i,j,nsurf),j=1,n2)
         enddo
